...
const locales = {
    ...
    es: {
        ...
        sendToNewDoc: "Enviar a Nuevo Documento",
        newDocSize: "Tamaño del Nuevo Documento",
        custom: "Personalizar Tamaño",
        width: "Ancho (px)",
        height: "Alto (px)",
        cancel: "Cancelar",
        create: "Crear",
        quickExport: "Exportación Rápida a PNG",
        layerAsPng: "Capa",
        docAsPng: "Documento",
...
function i18n(key) {
    const uxpPlugin = require("uxp");
    const langCode = (uxpPlugin.host && uxpPlugin.host.uiLocale) ? uxpPlugin.host.uiLocale : (navigator.language || "en");
...
async function handleExport(isEntireDoc) {
    if (app.documents.length === 0) {
        await app.showAlert(i18n("noDocOpen"));
        return;
    }
    ...
    try {
        ...
        await app.showAlert(i18n("errorPrefix") + (e.stack || e.toString() || "Unknown"));
    }
}
...
async function handleResize(isCrop) {
    ...
    try {
        ...
        await app.showAlert(i18n("errorPrefix") + (e.stack || e.toString() || "Unknown Error"));
    }
}
